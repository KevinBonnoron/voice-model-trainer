...
test_script_existence() {
    ...
    assert_file_exists "$SCRIPT_DIR/src/augment.sh" "Script augment.sh exists"
    assert_file_exists "$SCRIPT_DIR/src/metadata.sh" "Script metadata.sh exists"
...
test_script_permissions() {
    ...
    assert_success "[ -x '$SCRIPT_DIR/src/augment.sh' ]" "Script augment.sh is executable"
    assert_success "[ -x '$SCRIPT_DIR/src/metadata.sh' ]" "Script metadata.sh is executable"
...
test_help_output() {
    ...
    echo "$help_output" | grep -q 'augment'
    assert_success "[ $? -eq 0 ]" "Help mentions augment"
    echo "$help_output" | grep -q 'metadata'
    assert_success "[ $? -eq 0 ]" "Help mentions metadata"
...
test_script_syntax() {
    ...
    assert_success "cd '$SCRIPT_DIR/src' && sh -n augment.sh" "Augment script has valid syntax"
    assert_success "cd '$SCRIPT_DIR/src' && sh -n metadata.sh" "Metadata script has valid syntax"
